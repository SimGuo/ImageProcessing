...
int extract_features(int type){		//type = 0:sift, 1:surf

	//--------读取所有图片.得到bowTrainer：所有surf特征的合集
	cout << "extracting feature from the pictures....." << endl;
	for (int i = 0; i < 19; i++){
		...
		do {
			...
			if (type == 0) siftdetector.detect(tmp_image, tmp_keypoint);
			else surfdetector.detect(tmp_image, tmp_keypoint);
			//将对应的特征内容写入mat
			Mat tmp_feature;
			if (type == 0) siftextractor->compute(tmp_image, tmp_keypoint, tmp_feature);
			else surfextractor->compute(tmp_image, tmp_keypoint, tmp_feature);
	...
	ofstream tmpout;
	if (type == 0) {
		tmpout.open("siftbow.csv");
	}
	else { 
		tmpout.open("surfbow.csv"); 
	}
	for (int i = 0; i < 19; i++){
		long hFile = 0;
		string pathName, exdName;
...
void readData_and_train(int type){
	//--------第二波运行main相应的集合.
	//--------将all.csv里面的不同的类的数据分开成不同的测试集和训练集
	//split_train_test();

	CvMLData mlData;
	if(type == 0) mlData.read_csv("siftbow.csv");
	else mlData.read_csv("surfbow.csv");
